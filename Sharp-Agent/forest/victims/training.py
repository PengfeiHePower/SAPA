...
def run_step(kettle, poison_delta, epoch, stats, model, defs, optimizer, scheduler, loss_fn, pretraining_phase=False):
    ...
    if epoch % defs.validate == 0 or epoch == (defs.epochs - 1):
        ...
        source_acc, source_loss, source_clean_acc, source_clean_loss = check_sources(
            ...
            kettle.setup)
        #mody
        source_train_acc, source_train_loss, source_train_clean_acc, source_train_clean_loss = check_sources(
            model, loss_fn, kettle.source_trainset, kettle.poison_setup['target_train_class'],
            kettle.poison_setup['source_class'],
            kettle.setup)
        #mody
        print(f'Source train adv. loss is {source_train_loss:7.4f}, train fool  acc: {source_train_acc:7.2%} | '
