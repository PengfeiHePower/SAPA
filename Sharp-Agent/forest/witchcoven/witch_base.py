...
class _Witch():
    ...
    def _initialize_brew(self, victim, kettle):
        ...
        if self.args.source_criterion in ['cw', 'carlini-wagner']:
            ...
            self.source_grad, self.source_gnorm = victim.sharp_grad(_sources, _target_classes,criterion=criterion, selection=self.args.source_selection_strategy, sigma=self.args.sharpsigma)
        elif self.args.source_criterion in ['worstsharp']:
            criterion = nn.CrossEntropyLoss()
            self.source_grad, self.source_gnorm = victim.worst_sharp_grad(_sources, _target_classes,criterion=criterion, selection=self.args.source_selection_strategy, sigma=self.args.sharpsigma)
        else:
